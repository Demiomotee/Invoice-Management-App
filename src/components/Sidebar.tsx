...
export default function Sidebar() {
  ...
  return (
    <>
      <nav className="hidden lg:flex flex-col items-center justify-between w-[103px] bg-[#373B53] rounded-r-[20px] sticky top-0 h-screen z-50 flex-shrink-0" aria-label="Main navigation">
        <div className="w-full">
          <div className="w-[103px] h-[103px] bg-purple rounded-r-[20px] flex items-center justify-center relative overflow-hidden cursor-pointer">
            ...
            <img src={logoShape} alt="Invoice App logo" className="relative z-10 w-10 h-10 object-contain" />
          </div>
        </div>
        <div className="flex flex-col items-center gap-0 w-full">
          <button onClick={toggleTheme} className="w-full h-[72px] flex items-center justify-center hover:opacity-70 transition-opacity" aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}>
      ...
      <nav className="lg:hidden fixed top-0 left-0 right-0 h-[72px] bg-[#373B53] z-50 flex items-center justify-between" aria-label="Main navigation">
        <div className="w-[72px] h-[72px] bg-purple flex items-center justify-center relative overflow-hidden rounded-br-[20px] flex-shrink-0 cursor-pointer">
          ...
          <img src={logoShape} alt="Invoice App logo" className="relative z-10 w-8 h-8 object-contain" />
        </div>
        <div className="flex items-center h-full">
          <button onClick={toggleTheme} className="px-7 h-full flex items-center justify-center hover:opacity-70 transition-opacity" aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}>
            {theme === 'dark' ? <SunIcon /> : <MoonIcon />}
          </button>
          <div className="w-px h-full bg-[#494E6E]" role="separator" />
          <div className="px-6 h-full flex items-center justify-center">
            <div className="w-8 h-8 rounded-full overflow-hidden border-2 border-transparent hover:border-purple-light transition-colors cursor-pointer bg-[#7E88C3] flex items-center justify-center">
              <span className="text-white text-[10px] font-bold select-none">AB</span>
            </div>
          </div>
        </div>
      </nav>
