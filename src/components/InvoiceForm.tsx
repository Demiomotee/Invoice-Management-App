...
const inp = (err: boolean) =>
  `w-full px-5 py-4 rounded-[4px] bg-white dark:bg-dark-2 border text-xs font-bold text-dark-1 dark:text-white
   transition-colors outline-none focus-visible:ring-2 focus-visible:ring-purple
   ${err ? 'border-[#EC5757]' : 'border-gray-1 dark:border-dark-3 hover:border-purple focus:border-purple'}`
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      <div className="flex items-center justify-between">
        ...
        {error && (
          <span className="text-[10px] font-semibold text-[#EC5757] italic" role="alert">{error}</span>
        )}
      </div>
      {children}
    </div>
...
export default function InvoiceForm({ invoice, onClose }: Props) {
  ...
  const setItem = (index: number, field: keyof InvoiceItem, rawVal: string) => {
    setForm(prev => ({
      ...
      items: prev.items.map((item, i) => {
        ...
        const value = (field === 'quantity' || field === 'price') ? (parseFloat(rawVal) || 0) : rawVal
        const updated = { ...item, [field]: value }
        if (field === 'quantity' || field === 'price') {
          updated.total = (field === 'quantity' ? (parseFloat(rawVal) || 0) : updated.quantity) *
                          (field === 'price' ? (parseFloat(rawVal) || 0) : updated.price)
        }
        return updated
      }),
    }))
  ...
  const handleDraft = () => { addInvoice(form, true); onClose() }
    ...
    >
      ...
      <div
        ...
        className="
          ...
          w-full md:w-[616px] lg:w-[616px] lg:left-[103px]
      ...
      >
        <div className="flex-1 overflow-y-auto overscroll-contain px-6 pt-8 pb-4 md:px-14 md:pt-12">
          ...
          <div className="flex flex-col gap-6 mb-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              <Field label="City" error={errors.senderCity} id="senderCity">
                <input id="senderCity" className={inp(!!errors.senderCity)}
                  value={form.senderAddress.city} onChange={e => setField('senderAddress.city', e.target.value)} />
              </Field>
              <Field label="Post Code" error={errors.senderPostCode} id="senderPostCode">
                <input id="senderPostCode" className={inp(!!errors.senderPostCode)}
                  value={form.senderAddress.postCode} onChange={e => setField('senderAddress.postCode', e.target.value)} />
              </Field>
              <Field label="Country" error={errors.senderCountry} id="senderCountry" className="col-span-2 md:col-span-1">
                <input id="senderCountry" className={inp(!!errors.senderCountry)}
                  value={form.senderAddress.country} onChange={e => setField('senderAddress.country', e.target.value)} />
              </Field>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6 mb-10">
            <Field label="Client's Name" error={errors.clientName} id="clientName">
              <input id="clientName" className={inp(!!errors.clientName)}
              ...
              <div className="relative">
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                  <svg width="16" height="12" viewBox="0 0 16 12" fill="none" aria-hidden="true">
                    <path d="M14.667 0H1.333C.6 0 0 .6 0 1.333v9.334C0 11.4.6 12 1.333 12h13.334C15.4 12 16 11.4 16 10.667V1.333C16 .6 15.4 0 14.667 0zm0 2.667L8 6.667 1.333 2.667V1.333L8 5.333l6.667-4v1.334z" fill="#888EB0" />
                  </svg>
                </div>
              </div>
            </Field>
            ...
            <Field label="Street Address" error={errors.clientStreet} id="clientStreet">
              <input id="clientStreet" className={inp(!!errors.clientStreet)}
                value={form.clientAddress.street} onChange={e => setField('clientAddress.street', e.target.value)} />
            </Field>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
              <Field label="City" error={errors.clientCity} id="clientCity">
                <input id="clientCity" className={inp(!!errors.clientCity)}
                  value={form.clientAddress.city} onChange={e => setField('clientAddress.city', e.target.value)} />
              </Field>
              <Field label="Post Code" error={errors.clientPostCode} id="clientPostCode">
                <input id="clientPostCode" className={inp(!!errors.clientPostCode)}
                  value={form.clientAddress.postCode} onChange={e => setField('clientAddress.postCode', e.target.value)} />
              </Field>
              <Field label="Country" error={errors.clientCountry} id="clientCountry" className="col-span-2 md:col-span-1">
                <input id="clientCountry" className={inp(!!errors.clientCountry)}
                  value={form.clientAddress.country} onChange={e => setField('clientAddress.country', e.target.value)} />
              </Field>
            </div>
          </div>
                ...
                <div className="relative">
                  ...
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                      <path d="M14 2h-1V0h-2v2H5V0H3v2H2C.9 2 0 2.9 0 4v12c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H2V6h12v10zM4 8h2v2H4zm4 0h2v2H8zm4 0h2v2h-2z" fill="#7C5DFA" />
                    </svg>
                  </div>
                </div>
              </Field>
          ...
          <div>
            ...
            <div className="flex flex-col gap-5 md:gap-4">
              {form.items.map((item, i) => (
                <div key={i}>
                  <div className="md:hidden flex flex-col gap-4 pb-4 border-b border-gray-1 dark:border-dark-3 last:border-0">
                    ...
                    <div className="grid grid-cols-[64px_1fr_80px_16px] gap-4 items-end">
                      ...
                      <button onClick={() => setForm(p => ({ ...p, items: p.items.filter((_, idx) => idx !== i) }))}
                        className="text-gray-2 hover:text-[#EC5757] transition-colors self-end pb-3" aria-label="Remove item">
                        <TrashIcon />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="hidden md:grid grid-cols-[1fr_64px_104px_72px_16px] gap-4 items-center">
                    <input className={inp(!!errors[`item_name_${i}`])}
                      value={item.name} placeholder="Item name"
                      onChange={e => setItem(i, 'name', e.target.value)}
                    ...
                    <button onClick={() => setForm(p => ({ ...p, items: p.items.filter((_, idx) => idx !== i) }))}
                      className="text-gray-2 hover:text-[#EC5757] transition-colors" aria-label="Remove item">
                      <TrashIcon />
                    </button>
                  </div>
                </div>
            ...
            {errors.items && (
              <p className="text-[10px] font-semibold text-[#EC5757] mt-3" role="alert">{errors.items}</p>
            )}
        ...
        <div className="flex-shrink-0 bg-white dark:bg-dark-2 px-6 md:px-14 py-5 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] flex items-center gap-2">
          {!isEditing ? (
            <>
              <button onClick={onClose} className="px-6 py-4 rounded-3xl text-xs font-bold bg-[#F9FAFE] dark:bg-dark-3 text-gray-3 dark:text-gray-1 hover:bg-gray-1 dark:hover:bg-dark-1 transition-colors">
                Discard
              </button>
              <div className="flex-1" />
              <button onClick={handleDraft} className="px-6 py-4 rounded-3xl text-xs font-bold bg-[#373B53] text-[#888EB0] hover:bg-dark-1 transition-colors">
                Save as Draft
              </button>
              <button onClick={handleSubmit} className="px-6 py-4 rounded-3xl text-xs font-bold bg-purple text-white hover:bg-purple-light transition-colors">
                Save &amp; Send
              </button>
            </>
          ) : (
