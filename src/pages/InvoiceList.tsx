...
export default function InvoiceList() {
  ...
  return (
    <div className="max-w-[780px] mx-auto px-6 py-16 md:py-14 md:px-12">

      <header className="flex items-center justify-between mb-14 gap-4">
        <div>
          <h1 className="text-[32px] font-bold text-dark-1 dark:text-white tracking-tight leading-none">Invoices</h1>
          <p className="text-[13px] text-gray-2 mt-1.5">{subtitle}</p>
        </div>
        <div className="flex items-center gap-10">
          <Filter />
          <button
            onClick={() => setShowForm(true)}
            className="flex items-center gap-4 bg-purple hover:bg-purple-light transition-colors text-white rounded-3xl pl-2 pr-4 py-2 text-xs font-bold"
            aria-label="Create new invoice"
          >
            <span className="w-8 h-8 rounded-full bg-white flex items-center justify-center flex-shrink-0">
              <svg width="11" height="11" viewBox="0 0 11 11"><path d="M6.313 10.313V6.313h4V4.687h-4V.687H4.687v4H.687v1.626h4v4h1.626z" fill="#7C5DFA" /></svg>
            </span>
            ...
            <li key={invoice.id} style={{ animationDelay: `${i * 40}ms` }} className="animate-[slideUp_0.2s_ease_both]">
              <Link
                ...
                className="
                  ...
                  grid-cols-[80px_1fr_auto_auto_auto_auto]
                  max-md:grid-cols-2 max-md:grid-rows-3
                  bg-white dark:bg-dark-2 rounded-invoice px-8 py-7 max-md:p-6
                  border border-transparent hover:border-purple transition-colors
                  shadow-[0_10px_20px_rgba(72,84,159,0.1)] dark:shadow-[0_10px_20px_rgba(0,0,0,0.3)]
                  no-underline
                "
              ...
              >
                <span className="text-xs font-bold text-dark-1 dark:text-white max-md:row-start-1 max-md:col-start-1">
                  <span className="text-purple">#</span>{invoice.id}
                </span>
                <span className="text-[11px] text-gray-2 max-md:row-start-2 max-md:col-start-1">
                  Due {formatDate(invoice.paymentDue)}
                </span>
                <span className="text-[11px] text-gray-2 text-right max-md:row-start-1 max-md:col-start-2 max-md:text-right">
                  {invoice.clientName}
                </span>
                <span className="text-base font-bold text-dark-1 dark:text-white text-right max-md:row-start-3 max-md:col-start-1 max-md:text-left">
                  {formatCurrency(invoice.total)}
                </span>
                <StatusBadge status={invoice.status} />
                <svg className="text-purple max-md:hidden" width="7" height="10" viewBox="0 0 7 10" fill="none" aria-hidden="true">
